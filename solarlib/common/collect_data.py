...
def main():
    try:
        ...
        epoch_time_data = int(time.time()*1000)
        date_time_data = str(datetime.datetime.fromtimestamp(unix_time_now).strftime('%Y-%m-%d %H:%M:%S'))
        ...
        solar_data_base = DataBase('/media/solarpi/SOLARUSB/SolarMonitor/SolarDataCollection.db')
        solar_data_base.create_table()
        # date_time = "2020-04-21 20:25:00"
        solar_data_base.add_record(e_t=epoch_time_data, d_t=date_time_data, tm_v=values[0], tm_fv=values[1],
        ...
        solar_data_base.connection.commit()
